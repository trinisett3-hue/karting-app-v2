...
## État actuel (03/08/2026)

- **Domaine** : `trinisette.fr` acheté et délégué à Cloudflare. L'application tourne sur
...
- `karting-v2/supabase/functions/` — sources des Edge Functions **versionnées ici**. Une seule
  l'est aujourd'hui : `send-result-emails`. Les quatre autres déployées sur le projet Supabase
  (`create-checkout`, `billing-portal`, `stripe-webhook`, `provision-organization`) n'existent
  que côté plateforme — dette connue : leur source n'est nulle part dans ce dépôt, `get_edge_function`
  côté Supabase MCP est la seule façon de la lire. Cinq fonctions déployées au total.
- `karting-v2/supabase/functions/.env.example` — contrat des secrets attendus par les Edge
  Functions. Aucune valeur sensible, mais toute la liste. À tenir à jour à chaque ajout.
...
- Pas de branche `develop`, pas de Pull Requests. La CI existe depuis le 03/08 mais reste
  minimale (syntaxe JS + deux garde-fous d'URL) : ni tests, ni lint, ni build.
...
- Retour de paiement Stripe : les Edge Functions `create-checkout` et `billing-portal` ne
  codent en dur aucune URL (`successUrl` / `returnUrl` reçus du client, repli sur l'en-tête
  `Origin`). C'est donc la façade Lovable/React, hors dépôt, qui décide de la destination —
  et c'est elle qui pointe encore vers une URL de prévisualisation. À reprendre avec la vitrine.
